...
const UsersPage = () => {
  const { users, usersLoading, usersError, mutate } = fetchUsers();
  mutate();
    ...
    <tr key={item._id} className="even:bg-titusDarkLight hover:bg-black">
      <td className="flex items-center gap-4 p-4">
        ...
        <div className="flex flex-col">
          <h3 className="font-semibold text-gray-300">{item.first_name}</h3>
          <p className="text-sm text-gray-500">{item?.last_name}</p>
        </div>
      </td>
  ...
  return (
    <div className="bg-titusDarkGray p-4 rounded-md flex-1 m-4 mt-0">
      ...
      {usersError ? <ErrorWidget error={usersError} /> : null}
